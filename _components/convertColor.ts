...
interface typeRGBA extends typeRGB {
    a:number;
}
...
// 1 | --- CMYK ---------------------------------------------------------
colorConvertor.cmyk.rgb = (cmyk: typeCMYK): typeRGB => ({
...
// 2 | --- Grayscale -----------------------------------------------------
colorConvertor.grayscale.cmyk = (grayscale:number): typeCMYK => ({
    c: 0, m: 0, y: 0, k: grayscale,
})

colorConvertor.grayscale.rgb = (grayscale:number): typeRGB => {
    const _grayscale:number = Math.round((100 - grayscale) / 0.392156862745098)
    return { r: _grayscale, g: _grayscale, b: _grayscale }
}

colorConvertor.grayscale.w = () => ({ error: 'You can`t get the wavelength of no color' })

// 3 | --- Hex 3 ---------------------------------------------------------
colorConvertor.hex3.hex6 = (hex3:string):string => doubleString(hex3)

// 4 | --- Hex 4 -----------------------------------------------------
colorConvertor.hex4.hex8 = (hex4:string):string => doubleString(hex4)

colorConvertor.hex4.rgb = (hex4:string):typeRGB => {
    const _this = {
...
// 5 | --- Hex 6 -----------------------------------------------------
colorConvertor.hex6.hex3 = (hex6:string):string => {
    const reduce = (a:string, b:string):string => {
        const initJoin = [a, b].join('')
        const forceNumber = Math.floor(parseInt(initJoin, 16) / 16)
        const forceString = forceNumber.toString(16).toUpperCase()
        return forceString;
    }
    return reduce(hex6[0], hex6[1]) + reduce(hex6[2], hex6[3]) + reduce(hex6[4], hex6[5])
}

colorConvertor.hex6.hex4 = (hex6:string):string => (`${colorConvertor.hex6.hex3(hex6)}F`)

colorConvertor.hex6.hex8 = (hex6:string):string => (`${hex6}FF`).toUpperCase()

colorConvertor.hex6.rgb = (hex6:string):typeRGB => ({
...
// 6 | --- hex 8 -----------------------------------------------------
colorConvertor.hex8.rgb = (hex8:string):typeRGB => {
    const _this = {
...
colorConvertor.hex8.rgba = (hex8:string):typeRGBA => {
...
// 7 | --- html  -----------------------------------------------------
colorConvertor.html.rgb = (htmlInput:typeHTML):typeRGBA => PullDataFromList(html, 'rgb', htmlInput)

// 8 | --- hsl -----------------------------------------------------
colorConvertor.hsl.rgb = (_hsl:typeHSL):typeRGB => {
    const hsl = { ..._hsl }
    ...
    const x:number = c * (1 - Math.abs((hsl.h % 2) - 1))
...
colorConvertor.hsl.w = (hsl:typeHSL):number => Math.round(620 - ((170 / 270) * hsl.h))

// 9 | --- hsv -----------------------------------------------------
colorConvertor.hsv.rgb = (_hsv:typeHSV):typeRGB => {
    const hsv = { ..._hsv }
    hsv.h /= 360
    hsv.s /= 100
    hsv.v /= 100
...
// 10 | --- Lab -----------------------------------------------------
colorConvertor.lab.pantone = (labOrigin:typeLAB):typePantone => {
    const lab = { ...labOrigin }
    const _this = {
        index: 768,
        name: '',
...
// 16 | --- YUV -----------------------------------------------------
colorConvertor.yuv.rgb = (yuv:typeYUV):typeRGB => ({
